...
must_haves:
  truths:
    ...
    - "Repository contains source code without build artifacts, dependencies, or environment files visible on GitHub"
...
2. Update `package.json` -- keep all existing fields, add/modify:
   ...
   - Do NOT set `"repository"` here -- it will be set in Task 3 after the GitHub repo is created
...
- `git status` shows clean working tree (node_modules and build now ignored)
  ...
  <done>.gitignore excludes node_modules/build/.env, package.json has name/description/license/keywords (repository URL deferred to Task 3), both committed</done>
</task>
